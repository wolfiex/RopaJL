...
''' to i'''
def togephi(self,tmin = 1, tmax =144, edgelist = True, norm_flux = False,name='test'):
...
#
        ...
        if name==True : name = self.filename.replace('.nc','')+'_'+self.group+'_%s-%s'%(tmin,tmax)

        nx.write_gexf(G, name+ ".gexf")

        if edgelist: nx.write_edgelist(G,name+'.edgelist',comments = '# ', data = ['weight'] )
